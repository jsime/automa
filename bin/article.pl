...
sub usage {
    ...
    print <<EOU;
$name - Add and update article bodies in Automa.

Examples:

    $name --subject "An Insightful Article" --tag file

Options:

  -i <int>    ID of article to update. If not given, creates new one.

  -t <text>   Article's title.
...
publish an article if it does not have at least a title and above-the-fold
content.
